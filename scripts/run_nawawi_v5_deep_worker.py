...
def translate_nawawi_work(work_meta):
    ...
    trans_file = NAWAWI_TRANS / f"{slug}_v5_translated.json"
    epub_pure = NAWAWI_EPUBS / f"{slug}_pure_en.epub"
    epub_bilingual = NAWAWI_EPUBS / f"{slug}_bilingual_lexical_en.epub"
    
    print("\n" + "=" * 80)
    print(f"📖 STARTING IMAM NAWAWI WORK (v5 Sovereign Engine): {title_ar}")
    ...
    engine = LexicographicalTranslationEngine(
        author="Imam Abu Zakariyya Yahya ibn Sharaf al-Nawawi (d. 676 AH)",
...
def main():
    print("=" * 80)
    print("🌟 AYNENGINE AI v5.0.0: IMAM YAHYA IBN SHARAF AL-NAWAWI v5.0.0 RE-TRANSLATION PIPELINE")
    print("=" * 80)
    ...
    print(f"Loaded {len(works)} classical works for Imam Yahya ibn Sharaf al-Nawawi.\n")
    ...
    print("\n" + "=" * 80)
    print("🌟 AYNENGINE AI v5.0.0: ADVANCING TO IMAM AL-RAGHIB AL-ISFAHANI CORPUS")
    print("=" * 80)
    try:
        from scripts.run_raghib_v5_deep_worker import main as run_raghib
        run_raghib()
    except Exception as e:
        print(f"⚠️ Error executing Raghib pipeline: {e}")

    print("\n" + "=" * 80)
    print("🌟 AYNENGINE AI v5.0.0: ADVANCING TO CLASSICAL HERITAGE MASTERWORKS")
    print("=" * 80)
    try:
        from scripts.run_heritage_v5_deep_worker import main as run_heritage
        run_heritage()
    except Exception as e:
        print(f"⚠️ Error executing Heritage pipeline: {e}")
